# Sherpain21 스타일 가이드 v1.2

마지막 수정: 2025 | 작성: Sherpain21 Product Team
...
- **탑바 메뉴에는 아이콘을 사용하지 않는다. 텍스트만 표시한다.**
...
"신뢰 있는 실용 도구" x "한국형 데이터 마케팅 플랫폼" x "프리미엄 B2B SaaS"
...
- 14px 미만 본문 텍스트 금지
- 이모지 사용 금지 (아이콘은 Font Awesome 아이콘으로 대체)
...
**[아이콘 버튼 예시]**
```html
<button class="btn btn-primary" aria-label="무료로 시작하기">
...
**[Feature Card]**
- Base Card 상속
- `::before` pseudo: position absolute; top 0; left 0; right 0; height 3px; background `var(--gradient-accent)`; transform scaleX(0) → hover시 scaleX(1)
...
**[기능 목록 아이콘]**
- 체크 (포함): `<svg>` check 아이콘 색상 `var(--color-accent)`
- X (미포함): `<svg>` x 아이콘 색상 `var(--color-gray-400)`
...
- 높이: 64px
- 배경: rgba(13,27,42,0.95) + backdrop-filter: blur(12px)
- 스크롤 후: 배경 opacity 상승 + border-bottom 1px solid rgba(255,255,255,0.1)
- 모바일: 햄버거 아이콘(SVG) → 풀스크린 슬라이드다운 메뉴

**[GNB 구성]**
- 좌: 로고 (SVG 심볼 + 텍스트)
- 중: 메뉴 링크 (기능, 요금제, 미션마켓, 눈덩이, FAQ)
- 우: 로그인(텍스트) + 무료시작(btn-primary btn-sm)

### 6.7 대시보드 레이아웃

**[구조]**
- 상단 탑바: **텍스트 only** 메뉴 | 알림(bell SVG) | 유저(user SVG) — 높이 48px
- 좌측 사이드바: **260px (접힘 시 72px)**
  - 프로필 영역 (이름, 요금제, 눈덩이, 프로필/설정, 플랜/결제, 로그아웃)
  - 대시보드 (layout-dashboard SVG)
  - 플레이스 (map-pin SVG)
  - 블로그 (file-text SVG)
  - 미션 (target SVG)
  - 눈덩이 (zap SVG)
...
```
--chart-line-color: var(--color-accent)  (#2563EB)
...
| `--mindmap-l3` | `var(--color-gray-400)` | 3단계 (모자이크 대상) |
...
**[구조]**
- 레이블: "보유 눈덩이" (text-caption, gray-400)
- 금액: "1,240 눈덩이" (font-mono, 800 weight, gray-900)
- 환산: "약 12,400원" (text-caption, gray-400)
- 진행바: 현재 잔액 / 출금 최소(10,000 눈덩이) 퍼센트
- 버튼: 충전(btn-primary) | 출금(btn-dark, 조건부 disabled)
- 경고: 출금 최소 미달 안내 문구
...
| 방향 | 색상 | 텍스트 | 아이콘 |
|------|------|--------|--------|
| 상승 | `var(--color-success)` | "+3" | trending-up SVG |
| 하락 | `var(--color-danger)` | "-2" | trending-down SVG |
| 유지 | `var(--color-gray-400)` | "--" | minus SVG |
...
**[pulse-glow]** 라이브 상태 인디케이터
- 0%,100%: box-shadow 0 0 0 0 rgba(37,99,235,0.4)
- 50%: box-shadow 0 0 0 12px rgba(37,99,235,0)
- duration: 2s infinite

**[marquee]** 소셜 프루프 텍스트 띠
- 0%: translateX(0)
- 100%: translateX(-50%)
...
- API 호출 중: 버튼 disabled + 스피너(SVG animate) 또는 스켈레톤
- 순위 변동: 숫자 플립 애니메이션 (상승 초록, 하락 빨강)
- 눈덩이 차감: 잔액 숫자 카운트다운 애니메이션
- 에스크로 잠금: 카드 전체 반투명 오버레이 + lock SVG 아이콘
- 스크롤 진입 요소: IntersectionObserver 기반 fade-up
...
- 이미지: alt 텍스트 필수 (데코 이미지는 `alt=""` 빈 문자열)
...
- 터치 타겟 최소 크기: 44x44px
- 모바일 메뉴: `role="dialog"` `aria-modal="true"` 처리
- 포커스가 모달 외부로 나가지 않도록 Focus Trap 구현
...
- 스켈레톤 UI: 모든 API 응답 대기 화면에 필수 적용
- 이미지: WebP 우선, `loading="lazy"` 적용
- SVG 아이콘: 스프라이트 시트 또는 인라인 SVG 사용 (Font Awesome 아이콘 대비 렌더링 일관성)
- 번들: 코드 스플리팅, 기능별 청크 분리
...
| 페이지 | 섹션 구성 |
|--------|----------|
| 랜딩(홈) | Hero → 소셜프루프(마키) → 수치 통계 → 기능 그리드 → 무료기능 블록 → 요금제 → 에스크로 → 눈덩이 → FAQ → CTA |
| 대시보드 | 사이드바 + 메인: 빠른실행, 순위 카드, 눈덩이 위젯, 최근 미션, 활동 피드 |
| 플레이스 순위 | 키워드 입력 → 스켈레톤 → 순위 결과 카드 → 히스토리 차트 |
| 미션 마켓 | 필터 탭 → 미션 카드 그리드 → 에스크로 상태 배지 → 상세 모달 |
| 눈덩이 지갑 | 잔액 + 출금 버튼 → 충전 옵션 → 거래 내역 테이블 |
...
:root {
  ...
  /* ── Breakpoints (참조용, CSS에서는 media query로 직접 사용) ── */
  /* --bp-sm:   640px  */
  /* --bp-md:   768px  */
  /* --bp-lg:  1024px  */
  /* --bp-xl:  1280px  */
...
- **v1.2** 액센트 컬러 변경 (#00C9A7 민트 → #2563EB 딥 블루). TK/토큰 → 눈덩이 전체 변경. 탑바 메뉴 아이콘 제거(텍스트 only) 규칙 추가. 사이드바 접기/펼치기(260px↔72px) 스펙 추가. 사이드바 프로필 영역 재구성(이름/요금제/눈덩이/프로필설정/플랜결제/로그아웃). 탑바 높이 48px으로 변경. shadow/gradient 값 딥블루 기준 업데이트. gray 스케일 gray-50~gray-800 추가.

---

Version: 1.2.0
작성: Sherpain21 Product Team

본 스타일 가이드는 랜딩페이지, 대시보드, 미션 마켓 등 모든 서비스 UI에 일관되게 적용되어야 합니다. 컴포넌트 추가 시 반드시 이 문서의 토큰 시스템 내에서 확장하세요.
